//
//  WidgetView_Single01_M_Comp01(pad124x124).swift
...
struct WidgetView_Single01_M_Comp01_pad124x124_: View { // disable shadow
...
//                        Text("\(ddayNumber)")
                ...
                GeometryReader { metrics in
                    Text(ddaySymbolMenu[safe: 0]?.ddayDate ?? "2023.12.31")
                        ...
                        .position(x: metrics.size.width * 0.464, y: metrics.size.height * 0.75)
                        .foregroundColor(Color(hex: ddaySymbolMenu[safe: 0]?.ddayTxtColor ?? "ddayBlack"))
                }
            }
        }
